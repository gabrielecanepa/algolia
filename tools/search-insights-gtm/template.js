...
const INSIGHTS_LIBRARY_URL = 'https://cdn.jsdelivr.net/npm/search-insights@2.2.1';
...
function isValidReferer() {
  var refererUrl = getReferrerUrl('host');
  var hostUrl = getUrl('host');
  return (hostUrl.search('appspot.com') == -1) && (refererUrl.search('appspot.com') == -1);
}
// Domain check - end
...
switch (data.method) {
  ...
  case 'clickedObjectIDsAfterSearch': {
    ...
    const payload = {
      ...
      userToken: data.userToken,
      lp_ReferrerUrl1: getReferrerUrl('host'),
      lp_Url1: getUrl('host')
    };
  ...
  case 'clickedObjectIDs': {
    ...
    const payload = {
      ...
      objectIDs: formatValueToList(data.objectIDs),
      userToken: data.userToken,
      lp_ReferrerUrl2: getReferrerUrl('host'),
      lp_Url2: getUrl('host')
    };
    const chunks = chunkPayload(payload, ['objectIDs'], MAX_OBJECT_IDS);
  ...
  case 'clickedFilters': {
    ...
    const payload = {
      ...
      index: data.index,
      userToken: data.userToken,
      lp_ReferrerUrl3: getReferrerUrl('host'),
      lp_Url3: getUrl('host')
    };
    ...
    logger('sendEvents', chunks);
    if (isValidReferer()) {
      var checkFilter = data.filters[0]

      if ((checkFilter.search('undefined') == -1)) {
        aa('sendEvents', chunks);
      }
    }
    data.gtmOnSuccess();
